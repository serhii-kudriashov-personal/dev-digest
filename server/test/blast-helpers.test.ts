...
describe('foldBlastResult', () => {
  ...
  it('two changed symbols sharing a bare name (different files) get distinct `file`s and neither leaks the other\'s own-file caller', () => {
    const out = foldBlastResult({
      changedSymbols: [
        { name: 'renderWithIntl', file: 'src/a.test.tsx', kind: 'function' },
        { name: 'renderWithIntl', file: 'src/b.test.tsx', kind: 'function' },
      ],
      callers: [
        // Sits in `a.test.tsx` — must be excluded as a self-reference.
        { file: 'src/a.test.tsx', symbol: 'helperInA', viaSymbol: 'renderWithIntl', line: 5, rank: 3 },
        // Sits in `b.test.tsx` — must ALSO be excluded, even though the
        // exclusion map's first-seen file is `a.test.tsx`.
        { file: 'src/b.test.tsx', symbol: 'helperInB', viaSymbol: 'renderWithIntl', line: 6, rank: 2 },
        { file: 'src/c.ts', symbol: 'realCaller', viaSymbol: 'renderWithIntl', line: 1, rank: 1 },
      ],
    });
    expect(out).toHaveLength(2);
    expect(out[0]!.symbol).toBe('renderWithIntl');
    expect(out[0]!.file).toBe('src/a.test.tsx');
    expect(out[1]!.file).toBe('src/b.test.tsx');
    // Both entries share the same caller GROUP — the index resolves by name, not
    // by declaration id, so it cannot attribute `realCaller` to one file only —
    // but neither includes a caller sitting in either declaring file.
    expect(out[0]!.callers).toEqual([{ name: 'realCaller', file: 'src/c.ts', line: 1 }]);
    expect(out[1]!.callers).toEqual(out[0]!.callers);
  });
...
describe('summarizeBlast', () => {
  ...
  const downstream = [
    ...
    { symbol: 'beta', file: 'src/b.ts', callers: [], endpoints_affected: [], crons_affected: [] },
  ];
