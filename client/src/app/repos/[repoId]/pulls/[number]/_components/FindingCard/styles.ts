...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // PER-SIDE longhand. `borderColor` and `borderWidth` are THEMSELVES
    // shorthands for the four sides, so pairing either with a `borderLeft*`
    // longhand is the mix React warns about — dropping `border` was not enough.
    // It only surfaced for the colour, because that is the one that changes on
    // rerender (focus); the width pair was the same defect, silently.
    borderStyle: "solid",
    borderTopWidth: 1,
    borderRightWidth: 1,
    borderBottomWidth: 1,
