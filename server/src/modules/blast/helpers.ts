 ...
 *
 * `BlastCallerRow.viaSymbol` is a bare name — `getResolvedCallers` resolves
 * references by name, not by declaration id, so it cannot tell which of two
 * same-named changed symbols (declared in different files, e.g. two test
 * suites each with a local `renderWithIntl` helper) a given caller actually
 * reaches. Both entries therefore share the same caller group by construction;
 * `declFilesBySymbol` collects EVERY declaring file for a name (not just the
 * first) so the "own declaring file" exclusion below drops a self-reference
 * regardless of which of the same-named declarations it sits in. Each output
 * entry still carries its OWN `file`, so the wire shape stays disambiguable
 * even where the caller attribution cannot be.
 */
export function foldBlastResult(result: BlastResult): DownstreamImpact[] {
  const declFilesBySymbol = new Map<string, Set<string>>();
  for (const sym of result.changedSymbols) {
    const files = declFilesBySymbol.get(sym.name);
    if (files) files.add(sym.file);
    else declFilesBySymbol.set(sym.name, new Set([sym.file]));
  }
  const facts = result.factsByFile ?? {};

  const bySymbol = new Map<string, typeof result.callers>();
  for (const row of result.callers) {
    if (declFilesBySymbol.get(row.viaSymbol)?.has(row.file)) continue;
