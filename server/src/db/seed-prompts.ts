 ...
 *
 * Deliberately GENERIC about what makes a test weak. The specific rubric —
 * which branches to count, which boundaries to demand — is supplied by the
 * agent's linked SKILLS, so the same agent visibly improves when a skill is
 * attached. Adding that rubric here would defeat the L02 control experiment.
 */
...
/**
 * API Contract Reviewer (L02).
 *
 * As with the Test Quality prompt, the enumeration of what counts as a breaking
 * change lives in the agent's linked SKILLS rather than here.
 */
